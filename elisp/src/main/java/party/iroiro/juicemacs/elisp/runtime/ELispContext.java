...
public final class ELispContext {
    ...
    public static HashMap<MuleString, ELispSymbol> getObarrayInner(ELispVector obarray) {
        ...
        return inner == null ? new HashMap<>() : inner;
    }
    ...
    @CompilerDirectives.TruffleBoundary
    public static boolean unintern(ELispSymbol symbol, @Nullable ELispVector obarray) {
        if (obarray == null) {
            obarray = PSEUDO_OBARRAY;
        }
        HashMap<MuleString, ELispSymbol> inner = getObarrayInner(obarray);
        if (symbol.getInterned() == inner) {
            ...
            if (obarray.getFirst() == symbol) {
                Iterator<ELispSymbol> next = inner.values().iterator();
                obarray.set(0, next.hasNext() ? next.next() : false);
            }
            return true;
        }
        return false;
    ...
    public void initGlobal(ELispLanguage language) {
        ...
        initSymbols(variableSymbols());
        initSymbols(bufferLocalVarSymbols());
