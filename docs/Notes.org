...
The following example is inspired by [[https://scrapbox.io/shimizukawa/Unicode$(A$K%^%C%T%s%0$5$l$F$$$J$$NDWV(B][Unicode$(A$K%^%C%T%s%0$5$l$F$$$J$$NDWV(B - $(AGeK.(B
$(A4($N(BScrapbox]] ("Characters that are not yet mapped to Unicode" in English):

#+begin_src elisp
  (let* (;; $(AV^(B%G﨑%@$(AIqIg(B if the text were convertible to Unicode
    ...
    (format "Converted string: %s
  Original bytes:   %s
  Re-encoded bytes: %s
  Convertible char: %c (#x%x < #x10FFFF)
  Non-convertible:     (#x%x > #x10FFFF)"
            ...
            jis-encoded-bytes
            re-encoded-bytes
            convertible-char-1 convertible-char-1
            non-convertible non-convertible))
#+end_src

#+RESULTS:
: Converted string: $(AV^(B?$(AIqIg(B
: Original bytes:   $B='yu?@<R(B
: Re-encoded bytes: $B='yu?@<R(B
: Convertible char: $(AV^(B (#x6d32 < #x10FFFF)
: Non-convertible:     (#x1420a4 > #x10FFFF)
...
- [[https://www.cliki.net/cl-emacs][CL-Emacs]]: $(A!0(BVarious people have proposed an emacs-like editor written in Common
  Lisp. This page collects together a few possibilities.$(A!1(B
...
- [[https://blog.csdn.net/ftuc5dn/article/details/135421777][emacs $(AT4Bk7VNv#(6~#)(B]]: Contains a pretty diagram for some structs
...
[[https://www.graalvm.org/][GraalVM]]: $(A!0(BAn advanced JDK with ahead-of-time Native Image compilation$(A!1(B
...
- [[https://www.endoflineblog.com/graal-truffle-tutorial-part-0-what-is-truffle][Graal Truffle tutorial part 0 %G–%@ what is Truffle?]]: A series of Truffle tutorial on a JS-like language
