...
* TODO [#A] Start-up Performance Regression?
...
Currently, we use =internal-make-interpreted-closure-function= to determine
whether a closure captures the external environment or not: if not, we don't
need to materialized the parent frame. But otherwise, we really don't prune the
captured variables, which might leak memory if there are unused upper variables.
