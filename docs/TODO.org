...
- [ ] =kill-emacs=: This actually will not run code in =unwind-protect=, so a
  special exception probably will not do.
...
** TODO [#B] Buffer interval properties & markers & overlays
